...
- **Filet** : si le joueur n'a pas creusé 5 minutes après le seuil (en temps de strate), une fissure fine apparaît dans le tableau de bord. Elle s'allonge en 2 minutes, du graphique jusqu'au bouton du bandeau, sans texte ni blocage (elle ne capte aucun clic). Son tracé est fixe, irrégulier et ramifié, pour ne pas passer pour une seconde courbe. Le délai et la durée sont des constantes de `logique/regles.ts` (`DELAI_FISSURE`, `DUREE_FISSURE`). Le délai est à régler au playtest : le protocole du MVP ([#14](https://github.com/DavidGiangiacomo/strates/issues/14)) doit mesurer le temps entre le seuil et le premier coup de pioche.
